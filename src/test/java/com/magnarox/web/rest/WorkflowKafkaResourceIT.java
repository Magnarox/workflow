...
import com.magnarox.config.KafkaProperties;
...
class WorkflowKafkaResourceIT {
    ...
    void setup() {
        ...
        WorkflowKafkaResource kafkaResource = new WorkflowKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/workflow-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/workflow-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
